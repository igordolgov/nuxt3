...
.cart-page
  ...
  .container-lg.sm_px-4
    .cart-full.mx-auto.bg-white(v-if="!cartStore.isEmpty" class="md_w-3/4 lg_w-2/4")
      .mb-3.text-xl.text-center.font-semibold Корзина
      .bg-white.border.border-stone-400.px-3.py-2.mb-3.sm_px-4.shadow-md.shadow-stone-300.rounded-lg(
        ...
        .flex.justify-between.items-center.text-sm
          ...
          .description(class="min-w-[190px] md_min-w-[250px]")
            h5.md_text-lg {{item.name}}
            p.font-light {{item.price}} &#8381;
          .flex.m-0
      ...
      .flex.flex-col.bg-gray-600.sticky.z-10.inset-x-0.bottom-0.ml-auto.mr-2.px-4.py-2.rounded-lg.shadow-md.shadow-stone-400.text-white(class="w-[250px]")
        .flex.justify-between.text-stone-300
          p Сумма доставки:
          span(class="pr-[2px]") +1000 &#8381;
        .flex.justify-between
          p.font-bold Итого:
          span.font-bold {{cartStore.total}} &#8381;
    .cart-empty.text-center.my-auto.text-5xl.text-stone-600.font-bold.bg-cart.grid.place-items-center(v-else="") Корзина пуста
</template>
